...
function energy_balance_external_input_param(
    ...
    initial_conditions::Vector{InitialCondition},
    inflow_data::Vector{DeviceTimeSeriesConstraintInfo},
    upstream::Vector{Union{Nothing, Vector{String}}},
    cons_name::Symbol,
    var_names::Tuple{Symbol, Symbol, Symbol},
    param_reference::UpdateRef,
)
    ...
    for (ix, d) in enumerate(inflow_data)
        ...
        if !isnothing(upstream_devices)
            for name in upstream_devices
                JuMP.add_to_expression!(exp, varspill[name, 1], fraction_of_hour)
                JuMP.add_to_expression!(exp, varout[name, 1], fraction_of_hour)
            end
        end
        ...
        for t in time_steps[2:end]
            ...
            if !isnothing(upstream_devices)
                for name in upstream_devices
                    JuMP.add_to_expression!(exp, varspill[name, t], fraction_of_hour)
                    JuMP.add_to_expression!(exp, varout[name, t], fraction_of_hour)
                end
            end
